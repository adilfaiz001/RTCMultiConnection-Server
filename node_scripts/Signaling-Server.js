...
module.exports = exports = function(socket, config) {
    ...
    function onConnection(socket) {
         ...
         * Hand Raise Socket
         */
        socket.on('hand-raise', function(id, roomid, callback) {
            socket.broadcast.emit('hand-raised', id);
            callback();
        });
        ...
        socket.on('approved-audio', (id, callback) => {
            socket.broadcast.emit('on-approved-audio', id);
            callback(id);
        });
        ...
        socket.on('message', (messagePacket, roomid) => {
            var participants = listOfRooms[roomid].participants;
            ...
            participants.forEach((participant) => {
                ...
                listOfUsers[participant].socket.emit('receive-message', messagePacket);
            });

            // socket.broadcast.emit('receive-message', messagePacket);
            socket.emit('send-message');
        });

        socket.on('check-user', (userid, roomid) => {
            if (listOfRooms[roomid]) {

                // console.log(userid)
                // console.log(listOfRooms[roomid].participants);
                // console.log(listOfRooms[roomid].participants.length);

                // if (!listOfRooms[roomid].participants.includes(userid)) {
